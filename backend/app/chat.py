...
async def event_generator(user_id: str, query: str):
    # 保存用户输入
    await add_message(user_id=user_id, message={"role": "user", "content": query})
    # 获取上下文
    open_m = await get_messages(user_id=user_id)
    ...
    # 保存 AI 回复
    await add_message(user_id=user_id, message={"role": "assistant", "content": assistant_message})
...
async def chat(request: Request) -> StreamingResponse:
    ...
    return StreamingResponse(event_generator(user_id, content), media_type="text/event-stream")